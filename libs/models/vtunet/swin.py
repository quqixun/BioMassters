...
class WindowAttentionV1(nn.Module):
    ...
    def forward(self, x, mask, cross_qkv=None):
        b, n, c = x.shape
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix, prev_qkv=None):
        x_shape = x.size()
        x = self.norm1(x)
        if len(x_shape) == 5:
            b, d, h, w, c = x.shape
        ...
        attn_windows, qkv = self.attn(x_windows, mask=attn_mask)
        ...
        if self.apply_cross_attn:
            ...
            cross_attn_windows = self.attn(x_windows, attn_mask, cross_qkv)
            cross_attn_windows = cross_attn_windows.view(-1, *(window_size + (c,)))
            cross_shifted_x = window_reverse(cross_attn_windows, window_size, dims)
            if any(i > 0 for i in shift_size):
                if len(x_shape) == 5:
                    cross_x = torch.roll(cross_shifted_x, shifts=(shift_size[0], shift_size[1], shift_size[2]), dims=(1, 2, 3))
                elif len(x_shape) == 4:
                    cross_x = torch.roll(cross_shifted_x, shifts=(shift_size[0], shift_size[1]), dims=(1, 2))
            else:
                cross_x = cross_shifted_x

            if len(x_shape) == 5:
                if pad_d1 > 0 or pad_r > 0 or pad_b > 0:
                    cross_x = cross_x[:, :d, :h, :w, :].contiguous()
            elif len(x_shape) == 4:
                if pad_r > 0 or pad_b > 0:
                    cross_x = cross_x[:, :h, :w, :].contiguous()
    ...
    def forward(self, x, mask_matrix, prev_qkv=None):
        shortcut = x
        if not self.apply_cross_attn:
            ...
            if self.use_checkpoint:
                x, cross_x = checkpoint.checkpoint(self.forward_part1, x, mask_matrix, prev_qkv)
            else:
                x, cross_x = self.forward_part1(x, mask_matrix, prev_qkv)
            ...
            cross_x = shortcut + self.drop_path(cross_x)
            if self.use_checkpoint:
                cross_x = cross_x + checkpoint.checkpoint(self.forward_part2, cross_x)
            else:
                cross_x = cross_x + self.forward_part2(cross_x)
...
class PatchExpanding(nn.Module):
    ...
    def __init__(
        ...
        Args:
            input_resolution: resolution of input feature maps.
            dim:              number of feature channels.
            resample_scale:   resample scale at each dimention of input feature.
            norm_layer:       normalization layer.
            spatial_dims:     number of spatial dims.
        '''

        super().__init__()
        assert spatial_dims == 3, 'PatchExpanding supports spatial_dims 3 only'
        ...
        self.new_dims = shape_scale * dim // 2
        self.expand = nn.Linear(dim, self.new_dims, bias=False)
        self.output_dims = dim // 2
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            attn_drop:        attention dropout rate.
            norm_layer:       normalization layer.
            resample:         an optional resampling layer at the end of the layer.
            use_checkpoint:   use gradient checkpointing for reduced memory usage.
            apply_cross_attn: if apply cross attention.
        '''
...
class SwinTransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            ...
            input_size = layer.resample.output_resolution
            self.resamples.insert(0, layer.resample.resample_scale)
